...
function LeadModal({ isOpen, onClose, targetType, targetUrl }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (targetType === 'whatsapp') {
      const msg = `Solicito demo del Calentador de Cuentas.\n\nNombre: ${formData.name}\nEmpresa: ${formData.company}`;
...
export default function TutorialCalentadorPage() {
  ...
  const steps = [
    {
      ...
      description: "Haz clic en uno de los botones a continuación para descargar el instalador del Calentador de Cuentas.",
      content: (
      ...
      content: (
        <div className="mt-4 p-4 rounded-xl border border-white/10 bg-white/5 flex items-start gap-3">
          ...
          <p className="text-xs text-neutral-300 leading-relaxed">
            <strong className="text-orange-300">Importante:</strong> Todo el proceso de solicitud de token y activación se realiza directamente desde la interfaz del Calentador de Cuentas para mayor seguridad.
          </p>
        </div>
      )
    },
    {
      icon: CheckCircle,
      title: "4. Ingresar el Token y Calentar",
      description: "Cuando soporte te responda con tu Token, ingrésalo en la aplicación. ¡Listo! Ya puedes vincular tus cuentas de WhatsApp y comenzar a calentarlas de forma segura durante el periodo de prueba.",
      content: null
    }
  ];
    ...
    <div className="min-h-screen bg-[#050508] text-[#D7E2EA]">
      ...
      <main className="pt-32 pb-24 px-6 md:px-12 max-w-4xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          <a href={getPageUrl('calentador-cuentas')} className="inline-flex items-center gap-2 text-sm text-neutral-400 hover:text-white transition-colors mb-8 bg-white/5 px-4 py-2 rounded-full border border-white/10 hover:bg-white/10">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m15 18-6-6 6-6"/></svg>
            Volver a Calentador de Cuentas
          </a>
          ...
          <p className="text-neutral-400 text-sm md:text-base max-w-2xl">
            Sigue estos 4 sencillos pasos para descargar, instalar y activar tu licencia Demo del Calentador de Cuentas.
          </p>
        </motion.div>
